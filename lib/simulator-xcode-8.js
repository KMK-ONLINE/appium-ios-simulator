      ...
      await waitForCondition(async () => {
        try {
          ...
          if (stdout.trim().length > 0) {
            ...
            await exec('fbsimctl', [this.udid, 'launch', 'com.apple.springboard']);
            return true;
          }
        } catch (ign) {
      ...
      await waitForCondition(async () => {
        try {
          ...
          if (stdout.trim().length > 0) {
            ...
            await exec('fbsimctl', [this.udid, 'open', url]);
          } else {
            // Execute new Safari instance and open the url immediately
            await exec('fbsimctl', [this.udid, 'launch', 'com.apple.mobilesafari', url]);
          }
          return true;
        } catch (e) {
